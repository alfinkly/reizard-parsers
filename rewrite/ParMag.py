...
chrome_options.add_argument("--no-sandbox")  # Запуск Chrome без использования песочницы (необходимо в Docker и некоторых серверных средах)
chrome_options.add_argument("--disable-dev-shm-usage")  # Отключение использования разделяемой памяти, что является хорошей практикой в серверных средах
chrome_options.add_argument("--window-size=1920,1080")  # Установка размера окна, что важно для некоторых элементов страницы, требующих определённого разрешения
